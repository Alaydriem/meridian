...
use meridian::config::{parse_config_file, MeridianConfig};
...
fn build_client_from_config(config: &MeridianConfig, insecure: bool) -> Result<MeridianClient> {
    ...
    let base_url = format!("https://{}", api.listen);
...
pub async fn run_backend_command(
    ...
    let client = build_client_from_config(config, insecure)?;
...
pub async fn run_health(config: &MeridianConfig, insecure: bool) -> Result<()> {
    let client = build_client_from_config(config, insecure)?;
...
pub async fn run(cli: Cli) -> Result<()> {
    match &cli.command {
        ...
        Some(Command::Backend(args)) => {
            let config = parse_config_file(&cli.config)?;
            run_backend_command(&config, cli.insecure, args).await
        }
        Some(Command::Health) => {
            let config = parse_config_file(&cli.config)?;
            run_health(&config, cli.insecure).await
        }
    }
}
...
mod tests {
    ...
    fn test_build_client_missing_api_block() {
        ...
        let result = build_client_from_config(&config, false);
