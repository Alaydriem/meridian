...
use crate::udp::packet_router::PacketRouter;

use super::buffer_pool::BufferPool;
use super::completion::{OpType, UserDataTag};
use super::ephemeral_table::{EphemeralTable, SocketId};
...
impl RingWorker {
    ...
    pub fn run(
        ...
        Self::provide_all_buffers(&mut ring, &mut recv_pool)?;
        ...
        Self::submit_recv_msg_multi(&mut ring, &main_msghdr)?;

        // Worker-local: neither owns an fd.
        ...
        // Fixed indexes are ring-local, so allocation and reverse lookup must be too.
        // Index 0 is the main listening socket.
        let mut next_fixed_index: u32 = 1;
        let mut local_by_fixed_index: HashMap<u32, SocketId> = HashMap::new();
        ...
        loop {
                ...
                match tag.op_type {
                    OpType::MainRecvMulti => {
                        Self::handle_main_recv(
                            ...
                            cid_prefix_length,
                            &mut fixed_files,
                            &mut next_fixed_index,
                            &mut local_by_fixed_index,
                            &main_msghdr,
                            &shutdown,
                        );
            ...
            if last_cleanup.elapsed() >= cleanup_interval {
                eph_table.retire_expired();
                for (raw_fd, fixed_index, recv_buf) in eph_table.take_closable(worker_id) {
                    local_by_fixed_index.remove(&fixed_index);
        ...
        // Drain only the sockets this worker owns; peers close their own.
        let remaining = eph_table.drain_owned(worker_id);
    ...
    fn handle_main_recv(
        ...
        eph_table: &EphemeralTable,
        routing_table: &RoutingTable,
        crypto_buf: &CryptoReassemblyBuffer,
        cid_prefix_length: u8,
        fixed_files: &mut Vec<i32>,
        next_fixed_index: &mut u32,
        local_by_fixed_index: &mut HashMap<u32, SocketId>,
        ...
        Self::replenish_buffer(ring, recv_pool, buf_id);
        ...
        let client_addr = match Self::parse_sockaddr(name_data) {
            Some(addr) => addr,
            None => {
        ...
        // Get or create the ephemeral socket for this client. It may belong to a peer
        // worker, since the table is shared across rings.
        let target = match Self::ensure_ephemeral(
            worker_id,
            ring,
            recv_pool,
            eph_table,
        ...
        match target {
            ...
            EphTarget::Foreign { raw_fd, id } => {
                // Synchronous send on the raw fd: connected and non-blocking, so one
                // syscall that cannot block. `ensure_ephemeral` already claimed a send
                // reference, which holds off the owner's close until the release below.
                let sent = unsafe {
                    libc::send(
                        raw_fd,
                ...
                if sent < 0 {
                    ...
                    tracing::debug!(
                        worker = worker_id, raw_fd,
                        error = %err, "foreign ephemeral send failed"
                    );
                }
                // Must run on every path, or the owner can never close this fd.
                eph_table.release_send(id);
            }
        }
    }
    ...
    fn handle_eph_recv(
        ...
        eph_table: &EphemeralTable,
        recv_pool: &mut BufferPool,
        local_by_fixed_index: &HashMap<u32, SocketId>,
        ...
        // Completions are tagged with the ring-local fixed index. Read the client's
        // *current* address so a client that moved keeps receiving.
        let (id, client_addr) = match local_by_fixed_index
            .get(&fixed_index)
            .copied()
            .and_then(|id| eph_table.client_addr_for_id(id).map(|a| (id, a)))
        {
            Some(pair) => pair,
            None => {
        ...
        // Downstream traffic is activity: refreshing only upstream reaps connections busy
        // this way but idle upstream.
        eph_table.touch(id);

        // Forward data back to the client via the main socket (fixed index 0).
        if let Some(send_buf_id) = send_pool.alloc() {
            let n = send_pool.copy_into(send_buf_id, data);
            Self::queue_sendto_main(ring, send_pool, send_buf_id, n, client_addr);
        } else {
            tracing::debug!(
                worker = worker_id,
    ...
    ///
    /// `None` means the socket is indexed by client address alone: correct, but it loses
    /// rebind reuse.
    ...
    /// Ensure an ephemeral socket exists for the given client.
    #[allow(clippy::too_many_arguments)]
    fn ensure_ephemeral(
        worker_id: usize,
        ring: &mut Ring,
        ...
        // DCID first so a client that changed address keeps its socket; a new socket means
        // a new source port and another of the backend's five QUIC paths. The table is
        // shared, so this also finds sockets a *peer* created.
        if let Some(id) = eph_table.resolve(dcid, client_addr) {
            eph_table.touch(id);
            if eph_table.client_addr_for_id(id) != Some(client_addr) {
                eph_table.rebind_client(id, client_addr);
            }
            if let Some(d) = dcid {
                eph_table.register_dcid(id, d);
            }

            let target = eph_table.with_entry(id, |e| {
                if e.owner_worker == worker_id {
                    Some(EphTarget::Owned(e.fixed_index))
                } else if e.try_acquire_send() {
                    Some(EphTarget::Foreign {
                        raw_fd: e.raw_fd,
                        id,
                    })
                } else {
                    // Retired between resolve and now. Falling through would duplicate the
                    // socket for a live connection; drop and let the client retransmit.
                    None
                }
            });
        ...
        // Connect to backend.
        let (addr_ptr, addr_len) = Self::sockaddr_to_raw(&backend_addr);
        ...
        let fixed_index = *next_fixed_index;
        *next_fixed_index += 1;
        ...
        let id = eph_table.next_id();
        let entry = eph_table.new_entry(raw_fd, fixed_index, worker_id, client_addr, recv_buf_id);
        eph_table.insert(id, entry);
        local_by_fixed_index.insert(fixed_index, id);
        if let Some(d) = dcid {
            eph_table.register_dcid(id, d);
        }
        ...
        if let Some(buf_id) = recv_buf_id {
            Self::submit_eph_recv(ring, recv_pool, fixed_index, buf_id);
        }
        ...
        Some(EphTarget::Owned(fixed_index))
    }
    ...
    fn resubmit_eph_recv(
        ...
        Self::submit_eph_recv(ring, pool, fixed_index, buf_id);

        // Update the pending recv buf in the table.
        if let Some(id) = local_by_fixed_index.get(&fixed_index).copied() {
            eph_table.set_pending_recv(id, Some(buf_id));
        }
    }
    ...
    fn queue_sendto_main(
        ...
        let (addr_storage, addr_len) = Self::sockaddr_to_storage(&client_addr);
...
enum EphTarget {
    ...
    Owned(u32),
    /// A peer owns it, so its fixed index means nothing to our ring. Refcounted so the
    /// owner cannot close the fd underneath us: a recycled fd would put one tenant's
    /// data on another tenant's socket.
